...
# Usage: ./scripts/05-backup-commandcenter.sh [--check|--verify-restore]
...
# 6b. SOP stack config + SQLite data
log "Copying SOP stack config and data..."
SOP_DIR="$BACKUP_DIR/sop"
mkdir -p "$SOP_DIR"
cp -a "/opt/kimkom-sop/.env" "$SOP_DIR/" 2>/dev/null || true
cp -a "/opt/kimkom-sop/data" "$SOP_DIR/" 2>/dev/null || true
...
cat > "$BACKUP_DIR/manifest.json" << EOF
{
    ...
    "instances": [$(ls -d /opt/odoo-dev/*/ 2>/dev/null | xargs -I{} basename {} | sed 's/^/"/;s/$/",/' | tr -d '\n' | sed 's/,$//')],
