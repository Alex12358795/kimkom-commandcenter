#!/usr/bin/env bash
# 02-update-modules.sh — sync Odoo modules from CommandCenter to a customer VM
...
#
# Source of truth: /opt/odoo-modules/<slug>/ on CommandCenter (still git-tracked
# against the kimkom-modules repo).
# Target:          /opt/<slug>/modules/ on the customer VM.
set -Eeuo pipefail
...
usage() {
    cat <<'EOF'
Usage: 02-update-modules.sh --slug SLUG --target-ts-ip IP [options]
  --slug SLUG            Client slug (e.g. supertcg); sources /opt/odoo-modules/<slug>/
  --target-ts-ip IP      Customer VM Tailscale IP
  --upgrade-modules CSV  Comma-separated module list, or "all" (default: all)
  --stack-dir PATH       Customer stack directory (default: /opt/<slug>)
...
# Resolve defaults.
stack_dir=${stack_dir:-/opt/${slug}}
src=${src:-/opt/odoo-modules/${slug}}
