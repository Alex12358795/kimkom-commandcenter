#!/usr/bin/env bash
# 03-install-monitoring.sh — Add Prometheus exporters to an existing kimkom-deploy VM.
# Idempotent: run again to fix bind IP or update CC targets.
#
# Usage: 03-install-monitoring.sh --client-slug SLUG --target-ts-ip TS_IP
#
# Does:
...
if [[ "$have_node" -eq 0 || "$have_pg" -eq 0 ]]; then
    ...
    echo "Ensure the kimkom-deploy docker-compose.yaml is up to date (should include both services)." >&2
    exit 1
fi
...
add_target() {
    ...
    # Ensure the append starts on a fresh line: a file without a trailing
    # newline would merge the new block into the previous YAML mapping.
